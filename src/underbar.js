(function() {
  ...
  _.indexOf = (array, target, result = -1) => {
  ...
  _.filter = (collection, test, output = []) => {
  ...
  _.uniq = array =>
    _.reduce(array, (memo, item) =>
      memo.concat( (!memo.includes(item)) ? item : []), []);
  ...
  _.extend = function(obj) {
    _.each(arguments, (nextObj) =>
      _.each(nextObj, (value, key) =>
        obj[key] = value));
    return obj;
  };
  ...
  _.invoke = (collection, functionOrKey, args) =>
    _.map(collection, item =>
      typeof functionOrKey === 'string' ?
        item[functionOrKey].call(item, args) : functionOrKey.call(item, args));
  ...
  _.sortBy = (collection, iterator) =>
    collection.sort((a, b) =>
      (typeof iterator === 'string') ? a[iterator]  > b[iterator]: iterator(a) > iterator(b));
